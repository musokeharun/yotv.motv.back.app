...
router.use(function timeLog(req, res, next) {
    if (!req.path.includes("/login")) {
        let header = req.header("X-ADMIN-STATS-KEY");
        if (!header) {
        ...
        try {
            ...
            Db.ready(function () {
                Db.table("users").findSingle({email: decoded['user']}).then(function (user) {
                    console.log("Activity update")
                    Activity.lastActivity(user, "users", Db);
                }).catch(function (e) {
                    console.log("Error", e);
                })
            })
    ...
    Db.ready(() => {
        const saver = async () => {
            ...
            try {
                ...
                // } else
                res.status(403).send("Could not register,try again");
            }
        };
        saver().then((r) => {
        });
